...
static GraphBase* create_sample_undirected_graph() {
    ...
    City_t c;
    c.id = 1; std::strcpy(c.name, "北京"); g->add_vertex(c);
    c.id = 2; std::strcpy(c.name, "上海"); g->add_vertex(c);
    c.id = 3; std::strcpy(c.name, "广州"); g->add_vertex(c);
    c.id = 4; std::strcpy(c.name, "深圳"); g->add_vertex(c);
    c.id = 5; std::strcpy(c.name, "杭州"); g->add_vertex(c);
    g->add_edge(1, 2, 1200);
    g->add_edge(1, 3, 2100);
    g->add_edge(2, 3, 1400);
    g->add_edge(2, 5, 180);
...
static GraphBase* create_sample_dag() {
    ...
    City_t c;
    c.id = 1; std::strcpy(c.name, "A"); g->add_vertex(c);
    c.id = 2; std::strcpy(c.name, "B"); g->add_vertex(c);
    c.id = 3; std::strcpy(c.name, "C"); g->add_vertex(c);
    c.id = 4; std::strcpy(c.name, "D"); g->add_vertex(c);
    g->add_edge(1, 2, 1);
    g->add_edge(1, 3, 1);
    g->add_edge(2, 4, 1);
    g->add_edge(3, 4, 1);
...
static GraphBase* create_cyclic_digraph() {
    GraphBase* g = new AdjMatrix(10, GRAPH_DIRECTED);
    City_t c;
    c.id = 1; std::strcpy(c.name, "X"); g->add_vertex(c);
    c.id = 2; std::strcpy(c.name, "Y"); g->add_vertex(c);
    c.id = 3; std::strcpy(c.name, "Z"); g->add_vertex(c);
    g->add_edge(1, 2, 1);
    g->add_edge(2, 3, 1);
    g->add_edge(3, 1, 1);  /* 环路 */
    return g;
...
int test_add_city_boundary_max() {
    ...
    City_t c;
    for (int i = 0; i < max; ++i) {
        c.id = i + 1; c.name[0] = 'A' + i; c.name[1] = '\0';
        g->add_vertex(c);
    }
    c.id = 999; std::strcpy(c.name, "溢出");
    int result = g->add_vertex(c);
    delete g;
...
int test_dfs_disconnected() {
    GraphBase* g = new AdjMatrix(10, GRAPH_UNDIRECTED);
    City_t c;
    c.id = 1; std::strcpy(c.name, "孤岛1"); g->add_vertex(c);
    c.id = 2; std::strcpy(c.name, "孤岛2"); g->add_vertex(c);
    int* seq = nullptr;
    int len = 0;
...
int test_bfs_single_vertex() {
    GraphBase* g = new AdjMatrix(10, GRAPH_UNDIRECTED);
    City_t c;
    c.id = 1; std::strcpy(c.name, "单城"); g->add_vertex(c);
    int* seq = nullptr;
    int len = 0;
...
int test_dijkstra_no_path() {
    GraphBase* g = new AdjMatrix(10, GRAPH_UNDIRECTED);
    City_t c;
    c.id = 1; std::strcpy(c.name, "孤城A"); g->add_vertex(c);
    c.id = 2; std::strcpy(c.name, "孤城B"); g->add_vertex(c);
    int* dist = new int[2];
    int* prev = new int[2];
    int result = run_dijkstra(g, 1, dist, prev);
...
int test_floyd_compare_dijkstra() {
    ...
    for (int i = 0; i < vc; ++i) { delete[] (*floyd_dist)[i]; delete[] (*floyd_next)[i]; }
    delete[] * floyd_dist; delete[] * floyd_next;
    delete floyd_dist; delete floyd_next;
    delete[] dij_dist; delete[] dij_prev;
    delete g;
    return ok ? SUCCESS : ERR_INVALID_INPUT;
}
