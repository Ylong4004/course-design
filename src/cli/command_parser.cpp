                            ...
                            StructureComparator *comparator)
{
    ...
    if (argv.empty()) return true; /* 空行，继续 */
                             ...
                             StructureComparator *comparator)
{
    ...
    if (cmd == "new_city")       { cmd_new_city(argv, network); }
    else if (cmd == "del_city")  { cmd_del_city(argv, network); }
    else if (cmd == "new_road")  { cmd_new_road(argv, network); }
    else if (cmd == "del_road")  { cmd_del_road(argv, network); }
    else if (cmd == "update_road") { cmd_update_road(argv, network); }
    else if (cmd == "show_network") { cmd_show_network(network); }
    else if (cmd == "dfs")       { cmd_dfs(argv, network); }
    else if (cmd == "bfs")       { cmd_bfs(argv, network); }
    else if (cmd == "dijkstra")  { cmd_dijkstra(argv, network); }
    else if (cmd == "floyd")     { cmd_floyd(network); }
    else if (cmd == "prim")      { cmd_prim(network); }
    else if (cmd == "kruskal")   { cmd_kruskal(network); }
    else if (cmd == "topo")      { cmd_topo(network); }
    else if (cmd == "congest")   { cmd_congest(argv, network, simulator); }
    else if (cmd == "restore")   { cmd_restore(network, simulator); }
                                 ...
                                 RoadNetwork &network)
{
    ...
    if (rc == SUCCESS) {
        print_success(("添加道路 " + argv[1] + " -> " + argv[2]
                       + " (权值=" + argv[3] + ")").c_str());
                                    ...
                                    RoadNetwork &network)
{
    ...
    if (rc == SUCCESS) {
        print_success(("更新道路 " + argv[1] + " -> " + argv[2]
                       + " 权值=" + argv[3]).c_str());
    } else if (rc == ERR_ROAD_NOT_FOUND) {
        print_error("道路不存在。");
    } else {
                                 ...
                                 RoadNetwork &network)
{
    ...
    if (argv.size() >= 3 && is_integer(argv[2])) {
        ...
        int *path = nullptr;
        int plen = 0;
        if (dijkstra_get_path(prev, vcount, start, end, &path, &plen) == SUCCESS
            && plen > 0) {
            City_t c1, c2;
...
void CommandParser::cmd_floyd(RoadNetwork &network)
{
    ...
    /* 释放二维数组 */
    if (dist) {
        for (int i = 0; i < vcount; ++i) delete[] dist[i];
        delete[] dist;
    }
    if (next) {
        for (int i = 0; i < vcount; ++i) delete[] next[i];
        delete[] next;
    }
}
                                ...
                                CongestionSimulator *simulator)
{
    ...
    if (rc == SUCCESS) {
        print_success(("设置拥堵: " + argv[1] + " -> " + argv[2]
                       + " 权值=" + argv[3]).c_str());
    } else {
